      ...
      {selectedRemoteId && <section className="panel namespace-panel">
        ...
        {selectedNamespace && workspaceMappingState && <div className="workspace-mapping-console">
          ...
          <div className="workspace-path-list">{workspacePathEntries.map((entry) => <article key={entry.path} className={entry.cleanupEligible ? "workspace-path-cleanable" : ""}>
            ...
            {entry.mappings.map((mapping) => <div className="workspace-path-mapping" key={mapping.id}><span>{mapping.inherited ? "继承映射" : "路径映射"}：<code>{mapping.remotePrefix}</code> → <code>{mapping.localPrefix}</code></span>{!mapping.inherited && <button type="button" className="danger-button" onClick={() => void deleteWorkspaceMapping(mapping.id)} disabled={busy}>删除映射</button>}</div>)}
